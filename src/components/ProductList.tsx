...
const ProductList = async ({
  ...
  categoryID: string;
  limit?: number;
  searchParams?: any; // <- this now comes from the page props
}) => {
  const PRODUCT_PER_PAGE = 8;
  const wixClient = await wixClientServer();
  ...
  if (searchParams?.sort) {
    ...
    if (sortOrder === "asc") ProductQuery.ascending(sortField);
    if (sortOrder === "dsc") ProductQuery.descending(sortField);
  }
